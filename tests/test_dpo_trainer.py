...
def test_average_metrics() -> None:
    ...
    assert averaged.loss == 2.0
    assert averaged.implicit_kl == pytest.approx(0.2)
    assert averaged.accuracy == 0.75


def test_clone_reference_from_policy() -> None:
    policy = _TinyLM()
    reference = clone_reference_from_policy(policy)
    assert all(not param.requires_grad for param in reference.parameters())
    for left, right in zip(policy.parameters(), reference.parameters(), strict=True):
        assert torch.allclose(left.detach(), right.detach())


def test_trainer_runs_one_optimizer_step(tmp_path) -> None:
    policy = _TinyLM()
    reference = clone_reference_from_policy(policy)
    ...
    assert (tmp_path / "unit" / "dpo_training_summary.json").is_file()
