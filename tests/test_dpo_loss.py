...
class DpoLossTests(unittest.TestCase):
    def test_get_batch_logps_sums_completion_tokens_only(self) -> None:
        ...
        self.assertTrue(torch.allclose(logps[0], expected_0, atol=1e-5))
        self.assertTrue(torch.allclose(logps[1], expected_1, atol=1e-5))

    def test_get_batch_logps_average_mode(self) -> None:
        ...
        self.assertTrue(torch.allclose(avg, total / 3.0, atol=1e-5))

    def test_compute_dpo_loss_prefers_higher_chosen_margin(self) -> None:
        ...
        self.assertGreater(metrics.chosen_reward, metrics.rejected_reward)
        self.assertGreater(metrics.reward_margin, 0.0)
