...
class ExpertBank:
    ...
    def build_context(
        ...
        for (expert_index, _score), probability in zip(selected, probabilities, strict=True):
            ...
            sensitivity = clamp(
                ...
                + 0.35 * probability
                + deterministic_float(
                    f"expert:sensitivity:{prompt.prompt_id}:{expert_index}", -0.08, 0.08
                ),
                0.0,
                1.5,
            )
        ...
        normalized_entropy = clamp(
            router_entropy / max_entropy if max_entropy > 0.0 else 0.0, 0.0, 1.0
        )
        ...
        estimated_swap_cost_ms = sum(
            (1.0 - expert.resident_on_device)
            * (1.4 + 3.2 * expert.router_probability + 1.1 * (1.0 - expert.hotness))
    ...
    def _expert_hotness(
        self, prompt: PromptSample, hardware_profile: HardwareProfile, expert_index: int
    ) -> float:
        return clamp(
            0.35
            + 0.45
    ...
    def _resident_score(
        self, hardware_profile: HardwareProfile, expert_index: int, hotness: float
    ) -> float:
