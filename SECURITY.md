...
- **Hugging Face Hub downloads** ([`huggingface_cli.py`](src/adaptive_quant/huggingface_cli.py), route catalog): `repo_id`, filenames, and revisions are regex-validated; GGUF routes must use a `.gguf` filename. Set `ADAPTIVE_RL_HF_ALLOWED_REPOS` and/or `route_hf_allowed_repos` to restrict which repos may be fetched.
...
- **CI dev / PyTorch** installs use hash-pinned lockfiles **[`requirements/dev.txt`](requirements/dev.txt)** (includes `pip-audit`) and **[`requirements/pytorch-cpu.txt`](requirements/pytorch-cpu.txt)**; [`scripts/verify_lockfiles.py`](scripts/verify_lockfiles.py) checks inline hashes (regenerate via [`scripts/compile_locked_requirements.py`](scripts/compile_locked_requirements.py)).
- **Weekly pip-audit** on `main` via [`.github/workflows/pip-audit-scheduled.yml`](.github/workflows/pip-audit-scheduled.yml).
- **Router / online prompts** are length-capped (`MAX_ROUTER_TASK_TEXT_CHARS`) to limit tokenizer RAM exhaustion; analysis CLI paths reject `..` and control characters.
