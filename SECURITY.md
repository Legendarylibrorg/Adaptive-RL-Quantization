...
- **CI dev / audit / PyTorch** installs use hash-pinned lockfiles **[`requirements/dev.txt`](requirements/dev.txt)**, **[`requirements/audit.txt`](requirements/audit.txt)**, and **[`requirements/pytorch-cpu.txt`](requirements/pytorch-cpu.txt)** (regenerate via [`scripts/compile_locked_requirements.py`](scripts/compile_locked_requirements.py)).
