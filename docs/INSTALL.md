...
`./setup.sh` runs **hardware-aware setup tests** (a fast subset — not the full suite) and a short smoke (`config.e2e_smoke.json`). `./run` starts a **full** simulator run (uses `.venv/bin/adaptive-rl-quant` when the venv exists). You do not need to activate the venv first.
...
On **Linux + NVIDIA**, `./setup.sh` may require a secure-boundary ack first — see [SECURE_RUN.md](SECURE_RUN.md) or the 4090 example below.
...
On Linux + NVIDIA, `run_4090_pipeline.sh`, `./setup.sh`, and `scripts/install_cuda_torch.py` enforce the **NVIDIA secure boundary** in [SECURE_RUN.md](SECURE_RUN.md). Approve one tier before bootstrap or CUDA install. Example for a trusted lab host:
